...
class AllPost(db.Model):
    ...
    def by_id(cls, pid):
        # get post by ID
        return AllPost.get_by_id(pid, parent=blog_key())
...
class BlogFront(BlogHandler):
    def get(self):
        if not self.user:
            ...
            posts = db.GqlQuery("select * from AllPost order by "
...
class PostPage(BlogHandler):
    def get(self, post_id):
        if self.user:
            key = db.Key.from_path('AllPost', int(post_id), parent=blog_key())
            post = db.get(key)
...
class NewPost(BlogHandler):
    def get(self):
        # checks if you're a user, if yes,
        # renders the new post page, otherwise login
...
class EditPost(BlogHandler):
    ...
    def post(self, post_id):
        ...
        if post_subject and post_content:
            ...
            return
        # Subject and Content are required error
        else:
    ...
    def get(self, post_id):
        if self.user:
            ...
            if curr_user_id == post.creator:
                self.redirect('/blog')
                return
            elif curr_user_id not in post.likes:
